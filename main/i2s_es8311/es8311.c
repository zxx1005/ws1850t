...
static esp_err_t es8311_write_reg(es8311_ng_t *dev, uint8_t reg, uint8_t data)
{
...
static esp_err_t es8311_read_reg(es8311_ng_t *dev, uint8_t reg, uint8_t *data)
{
    ESP_ERROR_CHECK(
        i2c_master_transmit(dev->i2c_dev, &reg, 1, 100)
    );
    return i2c_master_receive(dev->i2c_dev, data, 1, 100);
}
#endif
...
es8311_ng_t *es8311_ng_create(void)
{
    es8311_ng_t *dev = calloc(1, sizeof(es8311_ng_t));
    if (!dev) return NULL;
